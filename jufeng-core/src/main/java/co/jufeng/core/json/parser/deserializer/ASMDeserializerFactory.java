...
import static co.jufeng.core.json.util.ASMUtils.getDesc;
import static co.jufeng.core.json.util.ASMUtils.getType;
...
import co.jufeng.core.json.asm.ClassWriter;
import co.jufeng.core.json.asm.FieldVisitor;
import co.jufeng.core.json.asm.Label;
import co.jufeng.core.json.asm.MethodVisitor;
import co.jufeng.core.json.asm.Opcodes;
...
import co.jufeng.core.json.parser.JSONLexerBase;
import co.jufeng.core.json.parser.JSONToken;
import co.jufeng.core.json.parser.ParseContext;
import co.jufeng.core.json.parser.ParserConfig;
import co.jufeng.core.json.parser.SymbolTable;
import co.jufeng.core.json.parser.DefaultJSONParser.ResolveTask;
import co.jufeng.core.json.parser.deserializer.ASMJavaBeanDeserializer.InnerJavaBeanDeserializer;
import co.jufeng.core.json.util.ASMClassLoader;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    void _deserialzeArrayMapping(ClassWriter cw, Context context) {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(co.jufeng.core.json.asm.Type.getType(getDesc(fieldClass)));
                ...
                if (itemClass == String.class) {
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(co.jufeng.core.json.asm.Type.getType(getDesc(itemClass)));
    ...
    void _deserialze(ClassWriter cw, Context context) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, getType(DefaultJSONParser.class), "getContext",
                           "()Lco/jufeng/json/parser/ParseContext;");
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                if (itemClass == String.class) {
                    mw.visitLdcInsn(co.jufeng.core.json.asm.Type.getType(getDesc(fieldClass))); // cast
    ...
    private void _deserialze_list_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_list_item_deser__",
                          getDesc(ObjectDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(co.jufeng.core.json.asm.Type.getType(getDesc(itemType)));
    ...
    private void _deserObject(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> fieldClass) {
        ...
        if (fieldInfo.getFieldType() instanceof Class) {
            mw.visitLdcInsn(co.jufeng.core.json.asm.Type.getType(getDesc(fieldInfo.getFieldClass())));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitLdcInsn(fieldInfo.getName());
