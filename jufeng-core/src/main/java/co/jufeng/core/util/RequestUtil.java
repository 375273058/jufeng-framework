...
public class RequestUtil {
	...
	public synchronized static <T> T toObject(HttpServletRequest request, Class<T> javaBean) throws Exception {
		return (T) RequestUtil.reflexObject(toObject(request), javaBean.newInstance(), javaBean);
	}
			...
			for (int i = 0; i < name.length; i++) {
				...
				if (data.containsKey(name[i]) && !StringUtils.isEmpty(value)
					...
					if (fields[i].getType() == java.lang.Long.class)
						...
						fields[i].set(object, DateUtil.toString(value, DateStyle.YYYY_MM_DD_HH_MM_SS.getValue()));
